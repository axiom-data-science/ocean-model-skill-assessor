...
def run(loc_model, approach, xarray_kwargs=None, time_range=None, parallel=True, readers=None, variables=None, criteria=None, var_def=None, local=None, erddap=None, only_search=False, skip_units=False, only_searchplot=False, stations=None):
    """Run package.
    ...
    criteria: dict, str, optional
        Note that this will both be used in this package and passed on to odg.
    var_def
    local
    erddap
    only_search
    ...
    kwargs = dict(criteria=criteria, var_def=var_def, approach=approach, parallel=parallel, variables=variables, readers=readers, local=local, erddap=erddap, skip_units=skip_units)
    
    bbox, boundary = find_bbox(dsm)
    ...
    if (len(search.dataset_ids) == 0) or only_search:
    ...
    for dataset_id in search.dataset_ids:
        ...
        if isinstance(search[dataset_id], pd.DataFrame):
            data = search[dataset_id]
        ...
        for variable in variables:
            ...
            df.omsa.plot(title=f'{dataset_id}: {stat_sum}', ylabel=ylabel,
                         figname=figname)

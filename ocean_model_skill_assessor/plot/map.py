...
from ..utils import find_bbox, open_catalogs, shift_longitudes, astype
...
def plot_map(
    ...
    if two_maps is not None:
        ...
        if "width_ratios" in two_maps:
            ...
            width_ratios = [1, 1]
        ax_map, ax = fig.subplots(1, 2, width_ratios=width_ratios, 
        ...
        ax_map.add_patch(mpatches.Rectangle(xy=[extent[0], extent[2]], 
    ...
    if inds.sum() > 0:
        if legend:
            ...
            kwargs_plot["color"] = colors_data#[inds]
        ax.plot(
            [min_lons[inds], max_lons[inds], max_lons[inds], min_lons[inds], min_lons[inds]],
            [min_lats[inds], min_lats[inds], max_lats[inds], max_lats[inds], min_lats[inds]],
            linestyle="-",
            linewidth=linewidth_data,
            transform=pc,
    ...
    if not legend or annotate:
        ...
        for i, (lon, lat, dd) in enumerate(zip(min_lons, min_lats, dds)):
            ...
            ax.annotate(label, xy=xyproj, xytext=xyproj, color=col_label, fontsize=annotate_fontsize)
    ...
    **kwargs_map,
    ...
    maps.extend(
        [
            ...
            for s in list(cat) if "minLongitude" in cat[s].metadata
        ]
    )
    # import pdb; pdb.set_trace()

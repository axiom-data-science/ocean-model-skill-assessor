...
def make_test_datasets():
    ...
    times = pd.date_range(str(example_loc.ocean_time.values[0]), 
                        str(example_loc.ocean_time.values[1]), freq="1H")
    npts = len(times)
    ...
    npts = 50
    df = pd.DataFrame({"date_time": '2009-11-19T14:00',
                ...
                "temperature": np.linspace(float(example_loc["temp"].max()), 
                                            float(example_loc["temp"].min()), npts),
                "salinity": np.linspace(float(example_loc["salt"].min()), 
                                            float(example_loc["salt"].max()), npts),
                })
    dds["profile"] = df
    ...
    times_full = np.hstack(([times[0]]*nptsperstation, 
                        [times[1]]*nptsperstation,
                        [times[2]]*nptsperstation, 
                        [times[3]]*nptsperstation, 
                        [times[4]]*nptsperstation
                    ))
    lons = np.linspace(float(example_loc1.lon_rho), float(example_loc2.lon_rho), nstations)
    ...
    df = df.reset_index()
    temp1 = np.linspace(float(example_loc1["temp"].max()), 
                        float(example_loc1["temp"].min()), nptsperstation)
    temp2 = np.linspace(float(example_loc2["temp"].max()), 
                        float(example_loc2["temp"].min()), nptsperstation)
    temp = np.hstack((temp1,
                    ...
                    temp2))
    salt1 = np.linspace(float(example_loc1["salt"].min()), 
                        float(example_loc1["salt"].max()), nptsperstation)
    salt2 = np.linspace(float(example_loc2["salt"].min()), 
                        float(example_loc2["salt"].max()), nptsperstation)
    salt = np.hstack((salt1,
                      salt1,
                      salt1,
                      salt2,
    ...
    lon = float(example_loc.lon_rho) + 0.01
    lat = float(example_loc.lat_rho) + 0.01
    temptemp = np.linspace(float(example_loc["temp"].max()), 
                        float(example_loc["temp"].min()), ndepths)
    temp = np.tile(temptemp[:,np.newaxis], [1,ntimes])
    salttemp = np.linspace(float(example_loc["salt"].min()), 
                        float(example_loc["salt"].max()), ndepths)
    ...
    # HF Radar
    example_area = ds.sel(eta_rho=slice(20,25), xi_rho=slice(10,15)).isel(ocean_time=0, s_rho=-1)
    temp = example_area["temp"].interp(eta_rho=[20,20.5, 21, 21.5, 22, 22.5, 23, 23.5, 24.5, 25],
                                    xi_rho=[10, 10.5, 11, 11.5, 12, 12.5, 13, 13.5, 14])
    salt = example_area["salt"].interp(eta_rho=[20,20.5, 21, 21.5, 22, 22.5, 23, 23.5, 24.5, 25],
                                    xi_rho=[10, 10.5, 11, 11.5, 12, 12.5, 13, 13.5, 14])
    lons = example_area["lon_rho"].interp(eta_rho=[20,20.5, 21, 21.5, 22, 22.5, 23, 23.5, 24.5, 25],
                                        xi_rho=[10, 10.5, 11, 11.5, 12, 12.5, 13, 13.5, 14])
    lats = example_area["lat_rho"].interp(eta_rho=[20,20.5, 21, 21.5, 22, 22.5, 23, 23.5, 24.5, 25],
                                        xi_rho=[10, 10.5, 11, 11.5, 12, 12.5, 13, 13.5, 14])
    dsd = xr.Dataset()
    dsd["temp"] = temp
    dsd["salt"] = salt

...
def test_kwargs_search_from_model(mock_xarray):
    ...
    output = {'min_lon': 0.0, 'max_lon': 9.0, 'min_lat': 0.0, 'max_lat': 9.0, 'min_time': '0', 'max_time': '9'}
    assert kwargs_search == output

    kwargs_search = {"min_time": 1, "max_time": 2, 
                     "model_path": "path"
                     }
    kwargs_search = omsa.utils.kwargs_search_from_model(kwargs_search)
    output = {'min_lon': 0.0, 'max_lon': 9.0, 'min_lat': 0.0, 'max_lat': 9.0, 'min_time': 1, 'max_time': 2}
    assert kwargs_search == output
    ...
    kwargs_search = omsa.utils.kwargs_search_from_model(kwargs_search)
    output = {'min_lon': 1, 'max_lon': 2, 'min_lat': 1, 'max_lat': 2, 'min_time': '0', 'max_time': '9'}
    assert kwargs_search == output
